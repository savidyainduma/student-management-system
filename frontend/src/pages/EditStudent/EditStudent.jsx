...
const EditStudent = ({selectedStudent, handleCloseEdit}) => {
  ...
  async function handleSubmit(event) {
    ...
    await axios.put('http://localhost:3001/api/students/editStudent/'+id, {FullName:name,BirthDate:bdate,Gender:gender,ContactNumber:number,Address:address,ParentContact:parent})
    .then(res =>{
      console.log(res)
      handleCloseEdit();
    })
    ...
    <div className='edit-student'>
      <form onSubmit={handleSubmit} className="edit-student-container">
        <div className="form-title">
            ...
            <img src={assets.cross_icon} onClick={handleCloseEdit} />
        </div>
        <div className="edit-student-input">
            <input type='text' value={name} placeholder='Full Name' onChange={e=> setName(e.target.value)} />
            <input type='date' value={bdate} placeholder='Date of Birth' onChange={e=> setBdate(e.target.value)} />
            ...
            <input type='text' value={number} placeholder='Contact Number' onChange={e=> setNumber(e.target.value)}/>
            <input type='text' value={address} placeholder='Address' onChange={e=> setAddress(e.target.value)}/>
            <input type='text' value={parent} placeholder='Parent`s Phone' onChange={e=> setParent(e.target.value)}/>
        </div>
